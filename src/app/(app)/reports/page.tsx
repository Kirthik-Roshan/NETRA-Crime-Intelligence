...
import { smartBrowzConfigured } from "@/lib/smartbrowz";
...
export default function ReportsPage({ searchParams }: { searchParams: { type?: string } }) {
  const t = getT();
  const type = searchParams.type || "district";
  const stats = dashboardStats();
  const byType = crimeByType();
  const hotspots = districtHotspots();
  const topCriminals = all<{ name: string; risk_score: number; home_district: string; crime_category: string }>(
    "SELECT name, risk_score, home_district, crime_category FROM criminals ORDER BY risk_score DESC LIMIT 10"
  );
  const recentCases = all<{ case_number: string; title: string; district: string; status: string; priority: string; updated_at: string }>(
    ...
    <div>
      <PageHeader title={t("reports.title")} subtitle={t("reports.subtitle")}>
        <div className="flex items-center gap-2">
          <a
            href={`/api/reports/${type}?format=pdf`}
            target="_blank"
            rel="noopener noreferrer"
            className="btn-accent"
            title={smartBrowzConfigured() ? "Server-generated PDF via Catalyst SmartBrowz" : "Branded PDF (opens print-ready document)"}
          >
            <FileDown className="h-4 w-4" /> {t("reports.branded_pdf")}
          </a>
          <PrintButton />
        </div>
      </PageHeader>
        ...
        {type === "trend" && (
          <ReportSection title="Crime Type Breakdown">
            <table className="w-full text-left text-sm">
              ...
              <tbody>
                {byType.map((t) => (
                  <tr key={t.crime_type} className="border-t border-border/50">
                    <td className="py-1.5">{t.crime_type}</td>
                    <td className="py-1.5 text-right font-mono">{t.count}</td>
                  </tr>
                ))}
              </tbody>
            </table>
